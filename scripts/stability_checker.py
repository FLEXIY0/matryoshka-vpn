    ...
    def score(self) -> float:
        ...
        if self.last_success:
            try:
                ...
                recency = max(0.0, 1.0 - age_h / 24)  # full score if checked <1h ago, zero after 24h
            except Exception:
                pass

        lat_score = max(0.0, 1.0 - self.latency_ms / 2000)  # 0ms→1.0, 2000ms→0.0
...
def parse_config(uri: str) -> Optional[ConfigRecord]:
    """Parse a proxy URI and extract host/port/protocol."""
    uri = uri.strip()
...
def tcp_test(host: str, port: int, timeout: float = TCP_TIMEOUT) -> float:
    """Return TCP connect latency in ms, or -1.0 on failure."""
    try:
        t0 = time.monotonic()
...
def _build_outbound(record: ConfigRecord) -> Optional[dict]:
    """Build xray outbound config dict from a proxy URI."""
    try:
        uri = record.uri
        ...
        if record.protocol == "vmess":
            ...
            if net == "ws":
                stream["wsSettings"] = {
                    "path": d.get("path", "/"),
                    "headers": {"Host": d.get("host", record.host)},
                }
            if d.get("tls") == "tls":
                stream["security"] = "tls"
                stream["tlsSettings"] = {
                    "serverName": d.get("sni", record.host),
                    "allowInsecure": False,
                }
            return {
                "protocol": "vmess",
        ...
        if transport == "ws":
            ...
            stream["grpcSettings"] = {
                "serviceName": p.get("serviceName", p.get("mode", "")),
            }
...
def export_top(records: list[ConfigRecord], n: int = TOP_N) -> int:
    """Write top-N configs to stable_top15.txt."""
...
def recheck_pool(pool: list[ConfigRecord], xray_bin: Optional[str]) -> list[ConfigRecord]:
    """Test every config currently in the stable pool."""
    if not pool:
        return pool
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=PROXY_WORKERS) as ex:
        ...
        for i, fut in enumerate(concurrent.futures.as_completed(futures), 1):
            ...
            if r.consecutive_failures == 0:
                tag = f"OK    {r.latency_ms:6.0f}ms"
            else:
                tag = f"FAIL  x{r.consecutive_failures}"
...
def scan_main_list(
    pool: list[ConfigRecord],
    xray_bin: Optional[str],
    needed: int,
) -> list[ConfigRecord]:
    """Scan russia_whitelist.txt for new stable configs to fill the pool."""
    ...
    all_uris = [
        u.strip()
        for u in MAIN_CONFIGS.read_text(encoding="utf-8").splitlines()
    ...
    # Stage 2a: TCP test all candidates concurrently (fast filter)
    ...
    # Stage 2b: Full proxy test on top TCP candidates
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Matryoshka VPN Stability Checker",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    parser.add_argument("--quick", action="store_true",
                        help="Recheck stable pool only, skip scanning main list")
    parser.add_argument("--scan", action="store_true",
                        help="Force scan main list even if pool is already full")
    parser.add_argument("--top", type=int, default=TOP_N,
                        help=f"How many top configs to export (default: {TOP_N})")
    parser.add_argument("--no-xray", action="store_true",
                        help="TCP-only mode, skip xray proxy tests")
    ...
    # Phase 1: Always recheck the existing pool
    pool = recheck_pool(pool, xray_bin)

    # Phase 2: Scan for new candidates if needed
    ...
    # Trim to max pool size (keep best by score)
    ...
    print(f"\n[+] Stable pool saved: {len(pool)} configs → {STABLE_POOL_FILE}")
    print(f"[+] Top-{n_exported} subscription file → {STABLE_TOP_FILE}")
